...
namespace simple_router
{
    ...
    {
        const RoutingTableEntry *best_entry = nullptr;
        uint32_t best_mask = 0u;
        for (const auto &entry : m_entries)
        {
            auto masked = ip & entry.mask;
            auto entry_masked = entry.dest & entry.mask;
            if (masked == entry_masked && entry.mask >= best_mask) // matched && longer prefix
            {
        ...
        if (best_entry)
        {
            fprintf(stderr, "RoutingTable lookup found best entry %s\n", ipToString(best_entry->dest).c_str());
            return *best_entry;
        }

...
namespace simple_router
{
    ...
    struct PendingPacket
    {
        ...
        std::string in_iface; //! New, The incoming interface
        ip_hdr ip_h;          //! New, Original IP header
    };
    ...
    public:
         ...
         * ! Notice: Return type changed
         */
        std::vector<std::tuple<uint32_t, Buffer>> periodicCheckArpRequestsAndCacheEntries();

        /**
         * Checks if an IP->MAC mapping is in the cache. IP is in network byte order.
         * You must free the returned structure if it is not NULL.
         */
        std::shared_ptr<ArpEntry> lookup(uint32_t ip);
         ...
         */
        std::shared_ptr<ArpRequest> queueRequest(uint32_t ip,
                                                 const Buffer &packet,
         ...
         */
        void removeRequest(const std::shared_ptr<ArpRequest> &entry);
